...
function navTo(location) {
    $('#mm').removeClass("on");
    $('#mmb').removeClass("on");
    $('html,body').animate({scrollTop: $("#" + location).offset().top}, 'slow'); //navigate to body
}

(function () {
    function run() {
        var segments = [], innerBody = document.querySelector(".innerBody"), leaves = document.querySelectorAll(".mobile-leaf"), src, newBase, newLeaf, i, node, base, inTemplate, len, leaf, seg;
        for (i = 0, len = leaves.length; i < len; i += 1) {
            ...
            while (node.tagName !== "BODY") {
                if (!base && /mobile-base/.test(node.className)) {
                    base = node
                }
                if (inTemplate && /holder/.test(node.className)) {
                    inTemplate = false
                }
            ...
            segments.push({
                ...
                isCode: leaf.className.match("code"),
                isH1: leaf.className.match("text") && leaf.querySelector(".textheading1"),
                isMenu: leaf.className.match("menuself")
            })
        }
        for (i = 0, len = segments.length; i < len; i += 1) {
            seg = segments[i];
            if (seg.inTemplate && !seg.isH1 && !seg.isMenu && !seg.isCode) {
                newLeaf = document.createElement("DIV");
                newLeaf.setAttribute("class", seg.leaf.className);
                src = newLeaf;
                if (seg.base) {
                    newBase = document.createElement("DIV");
                    newBase.setAttribute("class", seg.base.className);
                    newBase.appendChild(newLeaf);
                    src = newBase
                }
                seg.leaf.className += " mobile-hide";
                src.className += " mobile-show";
                innerBody.appendChild(src);
                seg.newLeaf = newLeaf
            }
        }
        function isMobile() {
        ...
        function move() {
            ...
            for (i = 0, len = segments.length; i < len; i += 1) {
                seg = segments[i];
                if (seg.newLeaf && seg.leaf.children.length) {
                    if (mobile) {
...
function hasScrolled() {
    ...
    if (st > lastScrollTop && st > navbarHeight) {
        // Scroll Down
        $('.mm').removeClass('nav-down').addClass('nav-up');
    } else {
        // Scroll Up
        if (st + $(window).height() < $(document).height()) {
            $('.mm').removeClass('nav-up').addClass('nav-down');
        }
    }
